...
export default function ShareButtons({ url = '', text = '', className = '', compact = false, forceDark = false, hideTelegram = false, hideCopy = false }) {
  ...
  const wa = forceDark
    ? { backgroundColor: 'rgba(37, 211, 102, 0.15)', border: '1px solid rgba(37, 211, 102, 0.2)', color: '#25D366' }
    : {};
  const tg = forceDark
    ? { backgroundColor: 'rgba(0, 136, 204, 0.15)', border: '1px solid rgba(0, 136, 204, 0.2)', color: '#0088cc' }
    : {};
  const tw = forceDark
  ...
  return (
    <div className={`flex items-center gap-2.5 ${className}`}>
      <button onClick={() => { track(EVENTS.SHARE_CLICK, { network: 'whatsapp' }); shareToWhatsApp(text, url); }}
        className={`${btnBase} hover:scale-110 ${forceDark ? '' : 'bg-[#25D366]/15 hover:bg-[#25D366]/25 border border-[#25D366]/20 hover:border-[#25D366]/40 text-[#25D366]'}`}
        style={wa} title="WhatsApp">
